...
description = "Overview of the Oreflow user interfaces (UIs)"
weight = 10
...
Your Oreflow deployment includes a central dashboard that provides quick access
to the Oreflow components deployed in your cluster. The dashboard includes the
following features:
...
## Overview of Oreflow UIs

The Oreflow UIs include the following:
...
* **Experiments (KFP)**: To manage [Oreflow Pipelines (KFP)](/docs/components/pipelines/) experiments.
...
provides access to the Oreflow
...
If you followed the guide to [deploying Oreflow on GCP](/docs/gke/deploy/),
the Oreflow central UI is accessible at a URL of the following pattern:
...
If you deploy Oreflow with Cloud Identity-Aware Proxy (IAP), Oreflow uses the
[Let's Encrypt](https://letsencrypt.org/) service to provide an SSL certificate
for the Oreflow UI. For troubleshooting issues with your certificate, see the
guide to
[monitoring your Cloud IAP setup](/docs/gke/deploy/monitor-iap-setup/).

## Using kubectl and port-forwarding

If you didn't configure Oreflow to integrate with an identity provider
...
Port-forwarding typically does not work if any of the following are true:
  ...
  * You've deployed Oreflow on GCP using the default settings
...
You can access Oreflow via `kubectl` and port-forwarding as follows:

1. Install `kubectl` if you haven't already done so:

    * If you're using Oreflow on GCP, run the following command on the command
...
1. Access the central navigation dashboard at:
    ...
    Depending on how you've configured Oreflow, not all UIs work behind
    port-forwarding to the reverse proxy.

    For some web applications, you need to configure the base URL on which
    the app is serving.

    For example, if you deployed Oreflow with an ingress serving at
...
* Explore the [contributor management
  ...
  multi-tenancy for your Oreflow deployment.
* [Set up your Jupyter notebooks](/docs/components/notebooks/setup/) in Oreflow.
